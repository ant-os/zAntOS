...
pub noinline fn init() !void {
    ...
    try root.serial.writer.print("All function symbols:\r\n", .{});
    si: for (symbols) |sym| {
        const typestr = switch (sym.st_type()) {
            std.elf.STT_FUNC => "function",
            std.elf.STT_OBJECT => "object",
            std.elf.STT_NOTYPE => "<no type>",
            else => continue :si,
        };

        try root.serial.writer.print(
            "{s} named \"{s}\" found starting at 0x{x} with size of {d} bytes\r\n",
            .{
                typestr,
                strtab_get(image, strtab, sym.st_name) orelse "<no name>",
                sym.st_value,
                sym.st_size,
            },
        );
    ...
    try root.serial.writer.print("Total of {d} ELF symbols found\n\r", .{symbol_count});
    try root.serial.writer.flush();
